...
const messages = {
  ...
  zh: {
    ...
    sidebar: {
      badge: "独立检索 UI",
      description: "一个界面完成语料准备、摄入、检索诊断和实验历史查看。",
      apiTarget: "本地 API 目标",
      footer: "FastAPI 服务需要单独运行。这个前端只通过 HTTP 调 deer-rag。",
      language: "语言",
      english: "EN",
      chinese: "中文",
      nav: {
        ...
        ingestLabel: "摄入",
        ingestDescription: "发送文本、URL 和文件",
        searchLabel: "检索",
        searchDescription: "运行 dense、BM25 和 hybrid",
        experimentsLabel: "实验",
    ...
    collectionsPage: {
      heading: "语料控制台",
      intro: "先创建逻辑上的收藏集，再进行摄入。后续 UI 里的所有操作都围绕一个 collection id 展开，所以这里是整条操作链路的起点。",
      newCollection: "新建收藏集",
      dialogTitle: "创建收藏集",
      dialogDescription: "第一版先保持简单。给语料命名，描述内容边界，只添加你后面真的会拿来过滤的 metadata。",
      totalCollections: "收藏集总数",
      stateTip: "收藏集本身很轻量。更好的方式是按实验或下游产品拆分语料，而不是把所有内容堆进一个大合集。",
      noCollectionsTitle: "还没有收藏集",
      noCollectionsDescription: "先创建一个收藏集，再去摄入页面添加文档并建立索引。",
      createFailed: "创建收藏集失败。",
      loadFailed: "加载收藏集失败。",
      createAction: "创建收藏集",
    },
    collectionCard: {
      noDescription: "还没有描述。这个收藏集已经可以开始做摄入和检索。",
      metadataFields: "项元数据",
      updated: "更新时间",
      metadataPreview: "元数据预览",
      noMetadataStored: "还没有元数据。",
      ingestIntoCollection: "向这个收藏集摄入",
      searchNow: "立即检索",
    ...
    ingestPage: {
      ...
      emptyTitle: "先从收藏集开始",
      emptyDescription: "这个页面需要先有收藏集。先去 Collections 页面创建一个，再回来把文本、URL 或文件摄入进去。",
      nextStepTitle: "显式建立索引",
      nextStepDescription: "摄入只会写入原始文档和 chunks。只有重新建立 dense 和 BM25 索引后，检索才能真正工作。",
      buildIndexes: "建立索引",
      chooseCollectionFirst: "请先选择收藏集。",
      buildFailed: "建立索引失败。",
      duplicateDetected: "检测到重复文档。",
      freshStored: "已写入新文档。",
      latestIngestEmpty: "提交左侧任一表单后，这里会立刻显示文档摘要。",
      latestBuildEmpty: "建立索引后，这里会显示 chunk 数量，以及 dense / lexical 写入条数。",
      loadCollectionsFailed: "加载收藏集失败。",
      preparing: "正在准备摄入工作区...",
      chunksCreated: "个 chunks 已创建",
      chunksAvailable: "个 chunks 可用",
    ...
    ingestForm: {
      ...
      chooseCollectionBeforeIngesting: "摄入前请先选择收藏集。",
    ...
    searchPage: {
      heading: "观察检索行为",
      intro: "这个页面故意偏诊断型。你可以直接看到 snippets、原始分数、改写后的 query 以及最终使用的检索策略。",
      chooseCollectionBeforeSearch: "检索前请先选择收藏集。",
      searchFailed: "检索失败。",
      runRetrieval: "执行检索",
      preparing: "正在准备检索工作区...",
    },
    searchResults: {
      ...
      firstQueryDescription: "先选择收藏集和策略，再执行检索，就能查看 snippets、排序分数和 query rewrite 结果。",
      latency: "延迟",
      resultCount: "结果数量",
      tokenEstimate: "Token 估算",
      queryVariants: "查询变体",
      untitledChunk: "未命名 chunk",
      score: "分数",
      chunk: "chunk",
      tokens: "tokens",
      noHitsTitle: "没有返回结果",
      noHitsDescription: "试试换一个收藏集、放宽查询，或者从 dense 切到 BM25 / hybrid。",
    },
    experimentsPage: {
      heading: "带着保存记录对比策略",
      intro: "在同一个页面里运行多策略对比、保存 artifact，并从历史栏重新打开旧实验，不需要再手写 curl。",
      loadFailed: "加载实验页面数据失败。",
      chooseCollectionBeforeRun: "运行实验前请先选择收藏集。",
      selectAtLeastOneStrategy: "至少选择一种策略。",
      runFailed: "实验运行失败。",
      loadArtifactFailed: "加载 artifact 失败。",
      artifactHistory: "Artifact 历史",
      noArtifacts: "先跑一次开启 `save_artifact` 的实验，这里才会出现历史记录。",
      latestRunTitle: "最近一次运行",
      latestRunEmpty: "发起一次实验后，这里会对比各个策略的延迟、token 用量和检索指标。",
      selectedArtifactLoading: "正在加载 artifact...",
      selectedArtifactTitle: "当前选中的 artifact",
      selectedArtifactEmpty: "从左侧历史栏选择任一 artifact，即可重新查看保存的实验内容。",
      runExperiment: "运行实验",
